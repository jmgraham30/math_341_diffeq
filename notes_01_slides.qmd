...
Several derivative notations will be used.[^deriv-notation]
...
or in standard form solved for the highest derivative:
$$y^{(n)} = f\!\left(t, y, y', \ldots, y^{(n-1)}\right)$$


## Linearity

An ODE is **[linear](https://en.wikipedia.org/wiki/Linear_differential_equation)** if it can be expressed as:
$$a_n(t)\,y^{(n)} + \cdots + a_1(t)\,y' + a_0(t)\,y = g(t)$$

where the $a_k(t)$ and $g(t)$ depend **only on $t$**, not on $y$ or its derivatives.
...
- With $y(0) = 1$: $C = 1$, so $y(t) = e^{2t}$ — **particular solution**
:::
...
#| fig-cap: "Family of solutions $y(t) = Ce^{2t}$ of $y' - 2y = 0$. Each curve is one member of the family; the highlighted trajectory (dashed black) is the particular solution $C=1$, corresponding to the initial condition $y(0)=1$."
#| fig-height: 4
...
With current $I = \dot{Q}$ and $V_C = Q/C$:

$$\boxed{L\ddot{Q} + R\dot{Q} + \frac{Q}{C} = E(t)}$$

**2nd-order, linear, constant coefficients** — identical in form to the damped driven oscillator!
