...
be absorbed into the $C_2 te^t$ term by redefining $C_2$.  In practice we
keep the particular solution as found and let the constants adjust when ICs
are applied.
:::
...
#| code-summary: "Show the code"
#| label: fig-ex2
#| fig-cap: "Example 2: $x''-2x'+x=e^t/t$ with $x(1)=0$, $x'(1)=0$. The particular solution $te^t(\\ln t-1)$ (green) grows rapidly for large $t$ since it inherits the $e^t$ factor of the homogeneous solutions. Red dots confirm the analytical formula."
