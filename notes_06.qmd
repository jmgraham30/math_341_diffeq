...
For the upper limit to vanish we need $a - s < 0$, i.e., $s > a$. Then:
...
- $X(s) = (1/s)e^s$: fails the $X(s)\to 0$ test as $s\to\infty$, so cannot be a Laplace transform.
...
| $f(t)H(t-a)$ | $e^{-as}\mathcal{L}[f(t+a)]$ | |
