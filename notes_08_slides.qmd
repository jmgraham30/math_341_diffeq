...
::: {.callout-tip}
Think of the integral as a *weighted accumulation of history*: it combines the past values of $x$ against a time-reversed, shifted copy of $y$. This memory structure arises naturally whenever a system's current output depends on all of its past inputs — exactly the behavior encoded in a forced DE.
:::
...
**Verification in the velocity problem:** for $t > a$,
$$v(t) = \frac{1}{m}\int_0^t e^{-(t-\tau)/m}\,\delta_a(\tau)\,d\tau = \frac{H(t-a)}{m}\,e^{-(t-a)/m}. \checkmark$$

Zero before the impulse, jumps to $1/m$ at $t=a$, then decays — exactly what a sharp blow should produce.
...
**Alternate derivation** — compute $\mathscr{L}[f_\varepsilon]$ and take $\varepsilon\to 0$:
