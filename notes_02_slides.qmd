...
For a first-order ODE $x' = f(t, x)$, at every allowable point $(t, x)$ in the plane the ODE prescribes the **slope** that any solution curve $x(t)$ through that point must have.
...
#| fig-cap: "Slope field for $x' = -x+2t$. Solutions converge toward the particular solution $x=2t-2$ (verification: $x'=2$ and $-x+2t=-(2t-2)+2t=2$ ✓)."
#| fig-height: 3.8
...
#| fig-cap: "Slope field for the autonomous ODE $x' = x(1-x/4)$. Equilibria at $x=0$ (unstable, red) and $x=4$ (stable, green). All solutions with $x_0>0$ converge to $x=4$."
#| fig-height: 3.8
...
#| fig-cap: "Slope field for $x' = x(x-t)$ with nullclines $x=0$ and $x=t$. Four solution curves are attempted; some exit the view window due to finite-time blowup."
#| fig-height: 3.8
...
::: {.callout-note}
SymPy's output is algebraically equivalent to the boxed formula but looks different. Verify they match:
```{python}
#| echo: true
standard = K_s / (1 + (K_s/x0_s - 1)*sym.exp(-r_s*t_s))
print("Equal:", sym.simplify(sol_log.rhs - standard) == 0)
```
:::
...
No choice of $C$ recovers $x=1$: setting $1+(t^2+C)^3=1$ requires $(t^2+C)^3=0$ for all $t$, which is impossible since $t^2$ varies. So $x=1$ is a **singular solution**.
:::
...
$$x' + p\,x = q(t) \quad\Longrightarrow\quad x(t) = e^{-pt}\int e^{pt}q(t) \,dt + C\,e^{-pt}$$

Note that here $p$ is a constant. 
...
# Analytical derivatives (avoids numerical artifacts at the switch point)
I1 = np.exp(-2*t1)
I2 = -2*Q_sw*np.exp(-2*(t2-2))
