...
3. Apply the **method of undetermined coefficients** to find particular solutions of nonhomogeneous equations.
...
where $p$ and $q$ are real constants and $f(t)$ is a given function called the **forcing term** (or source term).  When $f(t) \equiv 0$ the equation is **homogeneous**; otherwise it is **nonhomogeneous**.
...
**Structure of the general solution.** By the [Superposition Principle](https://en.wikipedia.org/wiki/Superposition_principle) (discussed in detail in [Topics 3](https://math341diffeq.netlify.app/topics_03)), the general solution of the nonhomogeneous equation (2) is:
$$
x(t) = x_h(t) + x_p(t),
$$
...
x'' + px' + qx = 0, \tag{H}
$$
we try the **ansatz** (educated guess) $x(t) = e^{\lambda t}$, where $\lambda$ is a constant to be determined.  The key insight is that differentiation merely scales $e^{\lambda t}$ by $\lambda$, so every derivative of $e^{\lambda t}$ shares the same exponential factor — substituting into a linear ODE with constant coefficients therefore reduces it to an algebraic equation for $\lambda$.  Substituting:
$$
...
When $\Delta = 0$, there is one repeated root $\lambda_1 = \lambda_2 = -p/2$.  The function $e^{\lambda_1 t}$ is one solution, but we need a second, linearly independent solution.  One can verify directly by substitution that $t\,e^{\lambda_1 t}$ is also a solution: if $x = te^{\lambda_1 t}$ then $x' = e^{\lambda_1 t}(1 + \lambda_1 t)$ and $x'' = e^{\lambda_1 t}(2\lambda_1 + \lambda_1^2 t)$, so
$$
x'' + px' + qx = e^{\lambda_1 t}\bigl[\lambda_1^2 t + 2\lambda_1 + p(1+\lambda_1 t) + qt\bigr] = e^{\lambda_1 t}\bigl[(\lambda_1^2+p\lambda_1+q)t + (2\lambda_1+p)\bigr] = 0,
$$
since $\lambda_1^2+p\lambda_1+q=0$ (characteristic equation) and $2\lambda_1 = -p$ (repeated root).  This technique of finding a second solution is called **reduction of order** and is developed in Logan §2.5.  The **general solution** is:
$$
...
For the underdamped case, the solution $x(t) = e^{-\gamma t}(C_1\cos\omega_d t + C_2\sin\omega_d t)$ can be written compactly in **amplitude-phase form**:
$$
x(t) = A\,e^{-\gamma t}\cos(\omega_d t - \phi),
$$
where the **amplitude** $A = \sqrt{C_1^2 + C_2^2}$ and the **phase** $\phi = \arctan(C_2/C_1)$.  This form makes the decaying envelope $Ae^{-\gamma t}$ and the oscillation frequency $\omega_d$ immediately explicit.
...
::: {.callout-important}
## Example 8b — Modification Rule with a Repeated Eigenvalue

Solve $x'' + 4x' + 4x = e^{-2t}$.

This example illustrates a different kind of overlap: the forcing is an exponential whose exponent **matches the repeated eigenvalue** of the homogeneous equation — not a frequency-matching resonance, but the same modification rule applies.

**Step 1 — Homogeneous solution.**

Characteristic equation: $\lambda^2 + 4\lambda + 4 = (\lambda+2)^2 = 0$, so $\lambda = -2$ (repeated).

$$x_h = (C_1 + C_2 t)\,e^{-2t}.$$

**Step 2 — First guess and why it fails.**

The natural guess for $f(t) = e^{-2t}$ is $x_p = Ae^{-2t}$.  But $e^{-2t}$ is already present in $x_h$ (with coefficient $C_1$).  A single multiplication by $t$ gives $Ate^{-2t}$, which is also present in $x_h$ (with coefficient $C_2$).  Since the eigenvalue is *repeated*, we must multiply by $t^2$:
...
$$x_p' = A(2t - 2t^2)\,e^{-2t}, \qquad x_p'' = A(2 - 8t + 4t^2)\,e^{-2t}.$$

Substituting into the ODE and factoring out $e^{-2t}$:
$$A(2 - 8t + 4t^2) + 4A(2t - 2t^2) + 4At^2 = 2A = 1 \;\Rightarrow\; A = \tfrac{1}{2}.$$

Note how all the $t$ and $t^2$ terms cancel automatically — this always happens when the modification rule is applied correctly.

**General solution:**
$$\boxed{x(t) = (C_1 + C_2 t)\,e^{-2t} + \tfrac{1}{2}t^2 e^{-2t}.}$$
:::

::: {.callout-tip}
## The Modification Rule: A Summary

| Situation | Standard guess | Modified guess |
|---|---|---|
| $f(t) = \cos(\omega t)$, $\omega = \omega_0$ (simple eigenvalue) | $A\cos\omega t + B\sin\omega t$ | $t(A\cos\omega t + B\sin\omega t)$ |
| $f(t) = e^{at}$, $a = \lambda_1$ (simple eigenvalue) | $Ae^{at}$ | $Ate^{at}$ |
| $f(t) = e^{at}$, $a = \lambda_1$ (repeated eigenvalue) | $Ate^{at}$ (still overlaps) | $At^2e^{at}$ |

In general: multiply the standard guess by the smallest power of $t$ needed to eliminate all overlap with $x_h$.
:::
...
$$
*Note:* This sign convention — with $W = x_1x_2' - x_2x_1'$ — matches the form used in Logan §2.4.  Some textbooks define $W$ with the opposite sign, which would swap the signs in the formula above; always verify against your reference.
:::
...
### Example 10 — Variation of Parameters with a Repeated Eigenvalue

Solve $x'' - 2x' + x = \dfrac{e^t}{t^2+1}$.

The forcing function $f(t) = e^t/(t^2+1)$ is not in the undetermined-coefficients table (the denominator $t^2+1$ is not a polynomial, exponential, or trig function), so we use variation of parameters.  This example is also important because the homogeneous equation has a **repeated eigenvalue**, giving a Wronskian of the form $e^{2t}$ rather than a constant — representative of what students will routinely encounter in Notes 5.

**Step 1 — Homogeneous solution.**

Characteristic equation: $\lambda^2 - 2\lambda + 1 = (\lambda-1)^2 = 0$, so $\lambda = 1$ (repeated).
...
We compute $x_1' = e^t$ and $x_2' = e^t + te^t = (1+t)e^t$, so:

$$W = e^t\cdot(1+t)e^t - te^t\cdot e^t = e^{2t}(1+t) - te^{2t} = e^{2t}.$$

This is a function of $t$, not a constant — it must be carried through the formulas correctly.

**Step 3 — Compute $u_1'$ and $u_2'$.**

$$u_1' = -\frac{x_2\,f}{W} = -\frac{te^t \cdot \dfrac{e^t}{t^2+1}}{e^{2t}} = -\frac{t}{t^2+1}.$$

$$u_2' = \frac{x_1\,f}{W} = \frac{e^t \cdot \dfrac{e^t}{t^2+1}}{e^{2t}} = \frac{1}{t^2+1}.$$

Note how the $e^{2t}$ factors from $x_1$, $x_2$, $f$, and $W$ cancel cleanly — this is why the method works even when $W$ is not a constant.

**Step 4 — Integrate.**

$$u_1 = -\int \frac{t}{t^2+1}\,dt = -\tfrac{1}{2}\ln(t^2+1).$$

$$u_2 = \int \frac{1}{t^2+1}\,dt = \arctan(t).$$

**Step 5 — Particular solution.**

$$x_p = u_1 x_1 + u_2 x_2 = -\tfrac{1}{2}e^t\ln(t^2+1) + te^t\arctan(t).$$

**General solution:**

$$\boxed{x(t) = (C_1 + C_2 t)\,e^t - \tfrac{1}{2}e^t\ln(t^2+1) + te^t\arctan(t).}$$

```{python}
#| code-fold: true
#| code-summary: "Show the code"

# SymPy verification for Example 10 by direct substitution.
# (SymPy's dsolve does not handle this forcing function symbolically,
#  so we verify by plugging x_p back into the ODE and confirming it equals f(t).)
t_sym = sym.Symbol('t', real=True)

xp = (-sym.Rational(1,2) * sym.exp(t_sym) * sym.log(t_sym**2 + 1)
      + t_sym * sym.exp(t_sym) * sym.atan(t_sym))

xp_prime  = sym.diff(xp, t_sym)
xp_prime2 = sym.diff(xp, t_sym, 2)

residual = sym.simplify(xp_prime2 - 2*xp_prime + xp - sym.exp(t_sym)/(t_sym**2 + 1))
print("Residual x_p'' - 2x_p' + x_p - f(t) =", residual)
...
#| code-summary: "Show the code"
#| label: fig-ex10
#| fig-cap: "Example 10: $x''-2x'+x=e^t/(t^2+1)$, $x(0)=0$, $x'(0)=0$. The analytical particular solution (blue) is confirmed by a direct numerical solve (red dots). The particular solution grows roughly like $te^t\\arctan(t)$, inheriting the exponential growth of the homogeneous solutions."

t_plot = np.linspace(0, 3, 400)

# Particular solution (u1*x1 + u2*x2)
xp_10  = (-0.5*np.log(t_plot**2 + 1)*np.exp(t_plot)
          + t_plot*np.arctan(t_plot)*np.exp(t_plot))
# With x(0)=0, x'(0)=0:
# x(0) = C1 + xp(0) = C1 + 0 = 0  =>  C1 = 0
# x'(t) = (C1 + C2*t)*e^t + C2*e^t + xp'(t)
# x'(0) = C1 + C2 + xp'(0); xp'(0) = 0  =>  C2 = 0
xh_10  = np.zeros_like(t_plot)   # C1=C2=0 for these ICs
x10_full = xh_10 + xp_10

def ode10_num(t, y):
    return [y[1], 2*y[1] - y[0] + np.exp(t)/(t**2 + 1)]

sol10n = solve_ivp(ode10_num, (0, 3), [0.0, 0.0], dense_output=True, max_step=0.01)
t_dots = np.linspace(0, 3, 20)

fig, ax = plt.subplots(figsize=(8, 4))
ax.plot(t_plot, x10_full, color='steelblue', lw=2.5,
        label=r'$x_p = -\frac{1}{2}e^t\ln(t^2+1) + te^t\arctan t$')
ax.plot(t_dots, sol10n.sol(t_dots)[0], 'ro', markersize=5, label='Numerical check')
ax.axhline(0, color='k', lw=0.5)
ax.set_xlabel('$t$'); ax.set_ylabel('$x(t)$')
ax.set_title(r"$x''-2x'+x=e^t/(t^2+1)$, $x(0)=0$, $x'(0)=0$")
ax.legend(fontsize=9)
plt.tight_layout()
plt.show()
```
...
The Wronskian $W = e^{2t}$ is a function of $t$, not a constant like the $W=2$ in Example 9.  However, because $x_1 = e^t$, $x_2 = te^t$, and $f = e^t/(t^2+1)$ all contain factors of $e^t$, those factors cancel in the numerators and denominator of the $u_1'$ and $u_2'$ formulas.  The remaining integrals, $\int t/(t^2+1)\,dt$ and $\int 1/(t^2+1)\,dt$, are standard calculus results.  This cancellation is characteristic of repeated-eigenvalue problems and is worth recognizing so you can simplify before integrating.
:::
...
::: {.callout-tip}
## Strategy
Always try undetermined coefficients first — it is faster when it applies.  Switch to variation of parameters when $f(t)$ contains $\ln t$, $\sec t$, $\tan t$, $1/t$, or any other function that does not appear in the guessing table above.
:::
...
| **Variation of parameters** | $u_1'x_1+u_2'x_2=0$, $u_1'x_1'+u_2'x_2'=f$; integrate; $W$ may be a function of $t$ (Examples 9–10) |
...
::: {.callout-note}
**Next:** Systems of first-order equations and the phase plane — Logan §3.1.
:::
