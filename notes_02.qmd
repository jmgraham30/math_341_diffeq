...
x' = f(t, x)
$$
has a natural geometric interpretation.  At any allowable point $(t, x)$ in the plane, the ODE tells us the *slope* $x'(t) = f(t, x(t))$ that a solution curve passing through that point must have.  If we draw a short line segment at each point with the prescribed slope, the collection of all such segments is called the **[slope field](https://en.wikipedia.org/wiki/Slope_field)** (or **direction field**) of the equation.
