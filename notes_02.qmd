...
::: {.callout-note}
This material corresponds to **Sections 1.1.3, 1.3, and 1.4** of @logan2015first.  Section 1.2 (separable equations as a special case of exact equations) is **not covered** in this course; we treat separable equations directly via separation of variables in the section below.
:::
...
A first-order ODE in standard form
$$
x' = f(t, x)
$$
has a natural geometric interpretation.  At any allowable point $(t, x)$ in the plane, the ODE tells us the *slope* $x'(t) = f(t, x(t))$ that a solution curve $x(t)$ passing through that point must have.  If we draw a short line segment at each point with the prescribed slope, the collection of all such segments is called the **[slope field](https://en.wikipedia.org/wiki/Slope_field)** (or **direction field**) of the equation.
...
Two families of curves are especially useful for understanding a slope field:
...
Python's `matplotlib.pyplot.quiver` function makes it straightforward to compute and plot slope fields.  The idea is to create a grid of $(t, x)$ points, evaluate $f(t, x)$ at each point, and draw a unit-length arrow in the direction $(1, f(t,x))$ (normalized so all arrows have equal length).
...
::: {.callout-note}
SymPy returns an algebraically equivalent but unsimplified form that may look quite different from the boxed expression above.  The reader can verify they are identical by algebraic manipulation, or via the following simplification:
```{python}
#| code-fold: true
#| code-summary: "Show the code"
...
::: {.callout-note}
**Domain restrictions.** The right-hand side requires $x \geq 0$ (for $\sqrt{x}$ to be real) and $t \neq 0$ (to avoid division by zero).  Since the initial condition is given at $t = 1 > 0$, the solution is valid on an interval of the form $(0, b)$ for some $b > 1$; we take $t > 0$ throughout.
:::
...
The ODE $x' = 6t(x-1)^{2/3}$ has the general solution $x(t) = 1 + (t^2 + C)^3$.  But $x(t) \equiv 1$ also satisfies the ODE (check: $x' = 0$ and $6t(1-1)^{2/3} = 0$ ✓), and *no* choice of $C$ in the general family gives $x(t) = 1$: setting $1 + (t^2 + C)^3 = 1$ requires $(t^2 + C)^3 = 0$ for all $t$, which is impossible since $t^2$ varies.  So $x = 1$ is a singular solution.

**Why uniqueness fails here.** Recall from Notes 1 that the Picard–Lindelöf theorem guarantees a unique solution when both $f(t,x)$ and $\partial f/\partial x$ are continuous near the initial point.  Here $f(t,x) = 6t(x-1)^{2/3}$, so
$$
\frac{\partial f}{\partial x} = 6t \cdot \tfrac{2}{3}(x-1)^{-1/3} = 4t(x-1)^{-1/3},
$$
which is **undefined** at $x = 1$.  The continuity condition on $\partial f / \partial x$ fails precisely along the singular solution $x = 1$, so the theorem's uniqueness guarantee does not apply there — and indeed we find two distinct solutions passing through any point $(t_0, 1)$: the constant solution $x \equiv 1$ and a solution from the general family with appropriately chosen $C$.
:::
...
$$
To see why this works, note that $\mu' = p(t)\mu$ (by the chain rule), so the product rule gives:
$$
(\mu x)' = \mu' x + \mu x' = p(t)\mu x + \mu x' = \mu(x' + p(t)x).
$$
Therefore
$$
...
The homogeneous part $x_h$ carries the initial condition (through $C$) and is often called the **transient**, because when $p(t) > 0$ (so that $P(t) \to \infty$ as $t \to \infty$) it decays away.  The particular part $x_p$ is driven by $q(t)$ and represents the **steady-state** response.
...
Integrating the right-hand side requires a standard trick: apply integration by parts **twice** to evaluate $\int e^{2t}\sin t\,dt$.

:::{.callout-tip collapse="true"}
## Integration by parts: computing $\int e^{2t}\sin t\,dt$

Let $I = \displaystyle\int e^{2t}\sin t\,dt$.

**First pass** — integrate by parts with $u = \sin t$, $dv = e^{2t}\,dt$, so $du = \cos t\,dt$, $v = \tfrac{1}{2}e^{2t}$:
$$
I = \tfrac{1}{2}e^{2t}\sin t - \tfrac{1}{2}\int e^{2t}\cos t\,dt.
$$

**Second pass** — apply integration by parts again to the remaining integral, with $u = \cos t$, $dv = e^{2t}\,dt$, so $du = -\sin t\,dt$, $v = \tfrac{1}{2}e^{2t}$:
$$
\int e^{2t}\cos t\,dt = \tfrac{1}{2}e^{2t}\cos t + \tfrac{1}{2}\int e^{2t}\sin t\,dt = \tfrac{1}{2}e^{2t}\cos t + \tfrac{1}{2}I.
$$

**Substitute back** and collect the $I$ terms on the left:
$$
...
$$
$$
\tfrac{5}{4}I = \tfrac{1}{2}e^{2t}\sin t - \tfrac{1}{4}e^{2t}\cos t
\quad\Longrightarrow\quad
\boxed{I = e^{2t}\!\left(\tfrac{2}{5}\sin t - \tfrac{1}{5}\cos t\right) + C.}
$$

The key move in this technique is recognizing that after two rounds of integration by parts the original integral $I$ reappears on the right-hand side, allowing you to solve algebraically for $I$.  This same trick works for any integral of the form $\int e^{at}\sin(bt)\,dt$ or $\int e^{at}\cos(bt)\,dt$.
:::
...
Each application in this section leads to a first-order linear ODE with constant coefficients of the form
$$
x' + p\,x = q, \qquad p, q \text{ constants},
$$
whose general solution with initial condition $x(0) = x_0$ is the unified formula
$$
\boxed{x(t) = x_{\infty} + (x_0 - x_{\infty})\,e^{-pt}}, \qquad x_{\infty} = \frac{q}{p}.
$$
Here $x_\infty$ is the stable equilibrium (the value approached as $t \to \infty$), and $1/p$ is the time constant governing how quickly the transient decays.  The three applications below are simply different physical interpretations of this same mathematical structure.
...
::: {.callout-tip}
The mechanical–electrical analogy noted in Notes 1 now becomes concrete.  The RC circuit equation and Newton's law of cooling are **mathematically identical** in form:

| Newton's Cooling | RC Circuit |
|:---|:---|
| $T' + hT = hT_e(t)$ | $Q' + \tfrac{1}{RC}Q = \tfrac{E(t)}{R}$ |
| Temperature $T$ | Charge $Q$ |
| Heat loss coefficient $h$ | $1/(RC)$ |
| Ambient temperature $T_e(t)$ | $E(t)/R$ |

Solutions to both share the same transient + steady-state structure.  Understanding one system gives immediate insight into the other — a hallmark of mathematical unification.
:::

---

### Chemical Reactor (Chemostat)

A well-mixed tank (reactor) of constant volume $V$ has a chemical of concentration $C(t)$.  Contaminated inflow at concentration $C_{\text{in}}$ enters at volumetric flow rate $q_v$, and the well-mixed solution exits at the same rate.  **Mass balance** (rate of change = rate in $-$ rate out) gives:
$$
V\,C' = q_v\,C_{\text{in}} - q_v\,C \quad\Longrightarrow\quad C' + \frac{q_v}{V}\,C = \frac{q_v}{V}\,C_{\text{in}}.
$$
This is a **first-order linear ODE** with $p = q_v/V$ and forcing term $(q_v/V)C_{\text{in}}$.  The integrating factor is $e^{q_v t/V}$, and with $C(0) = C_0$ the solution is
$$
C(t) = C_{\text{in}} + (C_0 - C_{\text{in}})\,e^{-q_v t/V}.
$$
...
<div style="position:relative; padding-bottom:56.25%; height:0; overflow:hidden;">
  <iframe style="position:absolute; top:0; left:0; width:100%; height:100%;"
          src="https://www.youtube.com/embed/zsOQ4lbeiWI?rel=0"
          title="YouTube video player"
          frameborder="0"
          allow="accelerometer; autoplay; clipboard-write;
                 encrypted-media; gyroscope; picture-in-picture"
