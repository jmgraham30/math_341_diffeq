...
Most differential equations cannot be solved analytically by a simple formula. In industry and science, differential equations are almost always solved numerically because most real-world models lead to equations too complicated to solve analytically. And, in the rare case that the problem can be solved analytically, the solution is often in the form of a complicated integral or infinite series which, to be practically useful, must be resolved by a computer calculation anyway. Logan remarks that "many would say that the material in this brief section is the most important in the entire book."
...
produce a sequence of **approximations** $X_0, X_1, X_2, \ldots, X_N$ at **discrete** times $t_0 < t_1 < \cdots < t_N = T$, that can be interpolated (*e.g.*, connected by straight line segments), that accurately represents the true solution $x(t)$.
...
defining the equally-spaced temporal grid

$$
t_n = t_0 + nh, \quad n = 0, 1, 2, \ldots, N.
$$

We seek a **recursive formula** that computes $X_{n+1}$ from $X_n$. Starting from $X_0 = x_0$, this formula is applied $N$ times to march from $t_0$ to $T$, yielding the **numerical solution**, that is, the sequence of points $\{(t_n, X_n)\}$.
...
The left side equals $x(t_{n+1}) - x(t_n)$ by the Fundamental Theorem of Calculus. The right side is approximated by the **left-hand (rectangle) rule**: replace $f(t, x(t))$ by its value at the left endpoint $t_n$:
